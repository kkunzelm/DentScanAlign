                          ...
                          const Params& params)
{
    ...
    const double cosMaxCrease =
        std::cos(params.maxCreaseAngleDeg * PI / 180.0);
...
std::vector<VertexDesc> getRegionVertices(const ScanData& scan,
                                           ...
                                           const Params& params)
{
    ...
    VertexDesc seedVertex = nearestVertex(mesh, seedPoint);
    ...
    FaceDesc sf = incidentFace(mesh, seedVertex);
    if (sf == SurfaceMesh::null_face())
        return {};
    ...
    while (!pq.empty()) {
        ...
        if (d > bestDist[f.idx()] + 1e-9) continue;
        if (expanded >= params.maxFacesPerSeed) continue;
        ++expanded;
    ...
    // Collect vertices from reached faces
    std::vector<bool> vertexInRegion(mesh.num_vertices(), false);
    for (auto f : mesh.faces()) {
        if (bestDist[f.idx()] >= 0.0) {
            for (auto vd : mesh.vertices_around_face(mesh.halfedge(f))) {
                vertexInRegion[vd.idx()] = true;
            }
        }
    }

    std::vector<VertexDesc> result;
    for (auto v : mesh.vertices()) {
        if (vertexInRegion[v.idx()])
            result.push_back(v);
    }
